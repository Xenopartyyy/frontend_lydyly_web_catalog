            ...
            try {
                var table = $('#produkTable').DataTable({
                    ...
                    ajax: {
                        ...
                        error: function(xhr, status, error) {
                            ...
                            $('#produkTable tbody').html(
                                '<tr><td colspan="12" class="text-center text-red-500 py-4">Gagal memuat data: ' +
                                error + '</td></tr>');
                        }
                    },
                    columns: [{
                                ...
                                if (type === 'display') {
                                    ...
                                    if (typeof data === "string") {
                                        try {
                                            ...
                                            console.error("Parse error fotobrg:", e, data);
                                            return '-';
                                        }
                                    ...
                                    // Format path: ltrim(str_replace('\\', '', trim($path)), '/')
                    ...
                    createdRow: function(row, data, index) {
                        $(row).addClass(index % 2 === 0 ? 'bg-pink-50' : 'bg-white').addClass(
                            'hover:bg-pink-100 text-center');
                    },
                    drawCallback: function() {
                        $('#produkTable thead').addClass('bg-pink-400 text-white');
                        $('.dataTables_length select, .dataTables_paginate .paginate_button').addClass(
                            'border border-pink-400 rounded-lg bg-pink-100 px-3 py-1 text-pink-800 hover:bg-pink-200'
                        );
                ...
                hideCustomLoading();
                $('#produkTable tbody').html(
                    '<tr><td colspan="12" class="text-center text-red-500 py-4">Gagal memuat data</td></tr>');
            }
        });
